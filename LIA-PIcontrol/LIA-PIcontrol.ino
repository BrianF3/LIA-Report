...
// Pushbutton pins
const int PBU = 6;  // Up button pin
const int PBD = 7;  // Down button pin
const int PBL = 8;  // Left button pin
const int PBR = 9;  // Right button pin

// Sensor and actuator pins
int sensor = A1;     // Sensor pin
int actuator = 10;   // Actuator pin

// Control variables
int CO = 0;             // Control output
int setPoint = 0;       // Set point
int PV = 0;             // Process variable (sensor reading)
int Error = 0;          // Error (difference between set point and process variable)
float pGain = 0.0;      // Proportional gain
float iGain = 0.0;      // Integral gain
int integralAction = 0; // Integral action for integral control
float EXCO;             // Scaled control output

// Time variables for control interval
unsigned long previousMillis = 0;  // Previous time stamp
const long interval = 100;         // Control interval in milliseconds
...
void loop() {
  ...
  switch (Screen) {
    ...
    case 2:
      ...
      // Additional code for the third LCD screen page
      lcd.setCursor(7, 0);
      lcd.print("Err=");
      lcd.print(Error);
      lcd.setCursor(0, 1);
